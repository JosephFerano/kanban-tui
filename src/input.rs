            ...
            KeyCode::Enter => {
                ...
                if task.is_edit {
                    if let Some(selected_task) = column.get_selected_task_mut() {
                        ...
                        db_conn.update_task_text(selected_task);
                    }
                } else {
                    let task = db_conn.insert_new_task(title, description, column);
                    column.add_task(task);
                    db_conn.set_selected_task_for_column(column.selected_task_idx, column.id);
                }
                *task_opt = None;
            }
            _ => (),
    ...
    match key.code {
        ...
        KeyCode::Char('J') => {
            if column.move_task_down() {
                ...
                state.db_conn.swap_task_order(task1, task2);
                let task_idx = column.selected_task_idx;
                let col_id = state.get_selected_column().id;
                state.db_conn.set_selected_task_for_column(task_idx, col_id);
            }
        ...
        KeyCode::Char('D') => {
            if !column.tasks.is_empty() {
                state.db_conn.delete_task(column.get_selected_task().unwrap());
                column.remove_task();
                let task_idx = column.selected_task_idx;
                let col_id = state.get_selected_column().id;
                state.db_conn.set_selected_task_for_column(task_idx, col_id);
